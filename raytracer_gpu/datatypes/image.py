...
class Image:
    ...
    def set_pixel(self, x: int, y: int, color) -> None:
        ...
        if not isinstance(color, torch.Tensor):
            color_tensor = torch.as_tensor(
                color, dtype=self.pixels.dtype, device=self.device
            )
        else:
            color_tensor = color
        ...
        if color_tensor.shape != (3,):
            raise ValueError(
                f"Color must have 3 elements (RGB), got {color_tensor.shape}"
            )
    ...
    def write_ppm(self, image_file):
        ...
        np_image = byte_tensor.numpy().transpose(
            1, 0, 2
        )  # PyTorch (W,H,C) -> PPM (H,W,C)
